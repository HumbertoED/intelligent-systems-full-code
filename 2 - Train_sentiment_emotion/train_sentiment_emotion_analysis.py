...
with open('data.csv', 'wb') as csvfile:
    fieldnames = ['Tweet', 'Class', 'Polarity', 'Agreement', 'Subjectivity', 'Confidence', 'Irony', 'Anger', 'Joy', 'Fear', 'Sadness', 'Surprise']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

    writer.writeheader()
    for z in range(0, 3000):
        writer.writerow({'Tweet': array[z], 'Class': array2[z], 'Polarity':polarity_array[z], 'Agreement':agreement_array[z], 'Subjectivity':subjectivity_array[z], 'Confidence':confidence_array[z], 'Irony':irony_array[z], 'Anger': anger_array[z], 'Joy': joy_array[z], 'Fear': fear_array[z], 'Sadness': sadness_array[z], 'Surprise': surprise_array[z]})
